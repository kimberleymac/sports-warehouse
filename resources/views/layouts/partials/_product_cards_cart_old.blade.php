...
@else
    ...
    {{-- Product Card --}}
                ...
                <article class="featured-products__product-card">
                    ...
                    <form action="{{ route("products.update-quantity", $item)  }}" method="POST">
                        @csrf
                        @method('PUT')
                        <input type="number" name="quantity" value="{{$item->quantity ?? 1}}" min="1" class="w-20" onchange="this.form.submit()">
                    </form>
